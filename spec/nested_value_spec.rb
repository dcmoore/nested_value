...
describe NestedValue do
  ...
  it "returns nil if you pass an access chain as an unsplatted array" do
    TestClass.nested_value(nested_hash, [:come_out, :come_out, :wherever, :you_are]).should == nil
  end

  it "allows for arrays to be keys" do
    some_weird_data = {[1,2] => "Found ME!!!"}
    TestClass.nested_value(some_weird_data, [1,2]).should == "Found ME!!!"
  end

  it "returns nil if you try to access an array with a symbol" do
    TestClass.nested_value([1,2,3], :a).should == nil
  end
end
